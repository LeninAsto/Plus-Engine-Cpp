...
void MainMenuState::LoadAssets(OpenGLESBackend& renderer) {
    // Backgrounds
    m_MenuBG.LoadGL(Paths::Image("menuBG"));
    ...
    m_MenuDesat.LoadGL(Paths::Image("menuDesat"));
...
void MainMenuState::Render(OpenGLESBackend& renderer) {
    if (!m_AssetsLoaded) {
        LoadAssets(renderer);
    }

    m_MenuBG.DrawGL(renderer);

    for (int i = 0; i < MENU_ITEM_COUNT; i++) {
        m_Items[i].DrawGL(renderer);
    }

    if (m_MenuDesat.alpha > 0.01f) {
        m_MenuDesat.DrawGL(renderer);
    }
}
