...
void FreeplayState::LoadAssets(OpenGLESBackend& renderer) {
...
void FreeplayState::DrawText(OpenGLESBackend& renderer, TTF_Font* font, const std::string& text,
                             int x, int y, SDL_Color color, bool centered) const {
    RenderText::Draw(renderer, font, text, x, y, color, centered);
}

void FreeplayState::Render(OpenGLESBackend& renderer) {
    if (!m_AssetsLoaded) {
        LoadAssets(renderer);
    }
    ...
    renderer.FillRect({0.0f, 0.0f, static_cast<float>(SCR_W), static_cast<float>(SCR_H)}, {0.0f, 0.0f, 0.0f, 120.0f / 255.0f});
    renderer.FillRect({40.0f, 70.0f, 760.0f, 570.0f}, {0.0f, 0.0f, 0.0f, 180.0f / 255.0f});
    renderer.FillRect({835.0f, 70.0f, 405.0f, 290.0f}, {0.0f, 0.0f, 0.0f, 170.0f / 255.0f});
    renderer.FillRect({0.0f, static_cast<float>(SCR_H - 78), static_cast<float>(SCR_W), 78.0f}, {0.0f, 0.0f, 0.0f, 190.0f / 255.0f});

    for (auto& label : m_Labels) {
        if (label.visible) {
            label.DrawGL(renderer);
        }
    }
    for (auto& icon : m_Icons) {
        if (icon.visible) {
            icon.DrawGL(renderer);
        }
    }
