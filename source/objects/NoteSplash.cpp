...
bool NoteSplash::s_TemplatesLoadedGL = false;
...
bool NoteSplash::EnsureTemplatesGL() {
    ...
    for (int lane = 0; lane < 4; ++lane) {
        for (int variant = 0; variant < 2; ++variant) {
            if (!s_Templates[lane][variant].LoadGL(imagePath, xmlPath)) {
                return false;
            }
...
bool NoteSplash::SpawnGL(const StrumNote& strum, int lane) {
...
void NoteSplash::DrawGL(OpenGLESBackend& backend) const {
    if (m_Alive) {
