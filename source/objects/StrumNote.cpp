...
bool StrumNote::LoadGL(int lane, bool player) {
    m_Lane = lane;
    m_Player = player;
    m_Pressed = false;
    m_ResetTimer = 0.0f;
    ...
    if (!m_Sprite.LoadGL(imagePath, xmlPath)) {
        return false;
    }

    m_BaseTextureGL = TextureCache::LoadGL(imagePath);
    m_PaletteTextureGL = RGBPalette::LoadNoteAtlasGL(imagePath, lane);
    if (!m_BaseTextureGL.IsValid() || !m_PaletteTextureGL.IsValid()) {
        return false;
    }
    ...
    m_LogicalWidth = m_Sprite.GetWidth();
    m_LogicalHeight = m_Sprite.GetHeight();
    m_Sprite.SetTexture(m_BaseTextureGL);
    RGBPalette::ApplyNeutralBrightness(m_Sprite, 1.0f);
    UpdateVisualPlacement();
    return true;
}
...
void StrumNote::DrawGL(OpenGLESBackend& backend) const {
...
void StrumNote::ApplyLanePalette(const std::string& animName) {
    ...
    if (m_PaletteTextureGL.IsValid()) {
        m_Sprite.SetTexture(m_PaletteTextureGL);
    }
