...
bool AnimatedSprite::LoadGL(const std::string& imagePath,
                             const std::string& xmlPath) {
    m_ImagePath = imagePath;
    m_GLTexture = TextureCache::LoadGL(imagePath);
...
void AnimatedSprite::DrawGL(OpenGLESBackend& backend) const {
    ...
    if (sf) {
        src = {
            static_cast<float>(sf->x),
            static_cast<float>(sf->y),
            static_cast<float>(sf->w),
            static_cast<float>(sf->h)
        };
        drawOffX = -(sf->frameX);
        drawOffY = -(sf->frameY);
    } else {
    ...
    if (src.x >= static_cast<float>(m_GLTexture.width) || src.y >= static_cast<float>(m_GLTexture.height)) {
        return;
    }

    const float clampedW = (std::min)(src.w, static_cast<float>(m_GLTexture.width) - src.x);
    const float clampedH = (std::min)(src.h, static_cast<float>(m_GLTexture.height) - src.y);
    const float ratioW = (src.w > 0.0f && clampedW < src.w) ? clampedW / src.w : 1.0f;
    const float ratioH = (src.h > 0.0f && clampedH < src.h) ? clampedH / src.h : 1.0f;
    src.w = clampedW;
    src.h = clampedH;
    ...
    cmd.source = src;
    cmd.dest = {
        x + static_cast<float>(drawOffX) * scaleX,
        y + static_cast<float>(drawOffY) * scaleY,
        static_cast<float>(sf ? sf->w : m_GLTexture.width) * scaleX * ratioW,
        static_cast<float>(sf ? sf->h : m_GLTexture.height) * scaleY * ratioH
    };
    cmd.color = {
...
float AnimatedSprite::GetWidth() const {
    ...
    if (m_GLTexture.IsValid()) return static_cast<float>(m_GLTexture.width) * scaleX;
    int tw = 0;
...
float AnimatedSprite::GetHeight() const {
    ...
    if (m_GLTexture.IsValid()) return static_cast<float>(m_GLTexture.height) * scaleY;
    int th = 0;
