...
bool Note::s_TemplatesLoadedGL = false;
...
bool Note::EnsureTemplatesGL() {
    ...
    for (int lane = 0; lane < 4; ++lane) {
        TextureHandle paletteTexture = RGBPalette::LoadNoteAtlasGL(imagePath, lane);
        if (!paletteTexture.IsValid()) {
            return false;
        }

        if (!s_HeadTemplates[lane].LoadGL(imagePath, xmlPath)) {
            return false;
        }
        if (!s_BodyTemplates[lane].LoadGL(imagePath, xmlPath)) {
            return false;
        }
        if (!s_TailTemplates[lane].LoadGL(imagePath, xmlPath)) {
            return false;
        }
...
bool Note::LoadGL(int lane, bool mustHit, float strumTime, float sustainLength, bool upScroll) {
...
void Note::DrawGL(OpenGLESBackend& backend) const {
    if (!m_Alive) {
        return;
    }
