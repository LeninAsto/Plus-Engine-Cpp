...
void DebugOverlay::Init(OpenGLESBackend& renderer) {
    (void)renderer;
    BuildFont();
}
...
void DebugOverlay::DrawString(OpenGLESBackend& renderer, int px, int py,
                               const std::string& s,
                               Uint8 r, Uint8 g, Uint8 b) const {
    SDL_Color color = { r, g, b, 255 };
    RenderText::Draw(renderer, m_Font, s, px, py, color, false);
}
...
void DebugOverlay::FillRect(OpenGLESBackend& rend, int x, int y, int w, int h,
                             Uint8 cr, Uint8 cg, Uint8 cb, Uint8 ca) {
    rend.FillRect({static_cast<float>(x), static_cast<float>(y), static_cast<float>(w), static_cast<float>(h)},
                  {static_cast<float>(cr) / 255.0f, static_cast<float>(cg) / 255.0f, static_cast<float>(cb) / 255.0f, static_cast<float>(ca) / 255.0f});
}
...
void DebugOverlay::Render(OpenGLESBackend& renderer) {
    ...
    if (m_Mode == Mode::VERBOSE) {
        ...
        lines.push_back("U/R/P: " + fmtF(app.GetUpdateMS(), 1) + " / " + fmtF(app.GetRenderMS(), 1) + " / " + fmtF(app.GetPresentMS(), 1) + " ms");
        lines.push_back(std::string("VSync: ") + (app.IsVSyncEnabled() ? "on" : "off"));
