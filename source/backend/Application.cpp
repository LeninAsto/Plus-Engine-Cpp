...
#include "../backend/debug/DebugOverlay.h"
#include <cmath>
#include <chrono>
#include <filesystem>
#include <vector>
...
bool Application::Init(const ApplicationConfig& config) {
    ...
    TextureCache::BindOpenGLESBackend(&m_Renderer);
...
bool Application::InitSDL() {
    ...
    if (!m_Renderer.Init(m_Window, m_Config.windowWidth, m_Config.windowHeight, m_Config.vsync)) {
        Logger::Error("Failed to initialize OpenGL ES renderer");
        return false;
    }
    Logger::Info("[OK] OpenGL ES renderer created" + std::string(m_Config.vsync ? " with VSync enabled" : ""));
...
void Application::Run() {
    ...
    while (m_Running) {
        ...
        Update(m_DeltaTime);
        m_UpdateMS = static_cast<float>(NowMS() - updateStartMS);

        Render();
        
        // Frame rate limiting is only used when VSync is disabled. If VSync is
        // active, SDL_GL_SwapWindow already owns pacing; adding another delay
        // can make a 60 Hz display feel like 30 Hz on some drivers.
        if (!m_Config.vsync) {
            const double frameTimeMS = NowMS() - frameStartMS;
...
void Application::CalculateTiming() {
    const double currentTimeMS = NowMS();
    if (m_LastFrameTimeMS <= 0.0) {
        m_LastFrameTimeMS = currentTimeMS;
    }

    m_DeltaTime = static_cast<float>((currentTimeMS - m_LastFrameTimeMS) / 1000.0);
    m_LastFrameTimeMS = currentTimeMS;
    m_LastFrameTime = SDL_GetTicks();
...
void Application::Render() {
    ...
    m_RenderMS = static_cast<float>(NowMS() - renderStartMS);
    const double presentStartMS = NowMS();
    m_Renderer.EndFrame();
    m_PresentMS = static_cast<float>(NowMS() - presentStartMS);
}
...
void Application::Shutdown() {
    ...
    DebugOverlay::Get().Destroy();
    MusicPlayer::Shutdown();
    SoundPlayer::Shutdown();
    TextureCache::Shutdown();
    TextureCache::BindOpenGLESBackend(nullptr);
    m_Renderer.Shutdown();
