...
void TitleState::LoadAssets(SDL_Renderer* renderer) {
    Alphabet::LoadAtlas(renderer);

    // Logo — no explicit scale (matches Haxe: logoBl has no setGraphicSize call)
    m_Logo.x = LOGO_X;
    m_Logo.y = LOGO_Y;
    if (m_Logo.Load(renderer, Paths::Image("logoBumpin"), Paths::Xml("logoBumpin"))) {
    ...
    m_GfDance.x = GF_X;
    m_GfDance.y = GF_Y;
    if (m_GfDance.Load(renderer, Paths::Image("gfDanceTitle"), Paths::Xml("gfDanceTitle"))) {
    ...
    if (m_TitleEnter.Load(renderer, Paths::Image("titleEnter"), Paths::Xml("titleEnter"))) {
...
void TitleState::Render(SDL_Renderer* renderer) {
    ...
    m_GfDance.Draw(renderer);
    m_Logo.Draw(renderer);
    m_TitleEnter.Draw(renderer);

    // Black overlay during intro
    if (!m_SkippedIntro && m_BlackAlpha > 0.0f) {
        SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
        SDL_SetRenderDrawColor(renderer, 0, 0, 0,
            static_cast<Uint8>(m_BlackAlpha * 255.0f));
        SDL_Rect full = { 0, 0, SCR_W, SCR_H };
    ...
    if (m_DoFlashOut && m_FlashTimer < 2.0f) {
        ...
        Uint8 flashA   = static_cast<Uint8>((1.0f - progress) * 255.0f);
        SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
        SDL_SetRenderDrawColor(renderer, 255, 255, 255, flashA);
        SDL_Rect full = { 0, 0, SCR_W, SCR_H };
        SDL_RenderFillRect(renderer, &full);
        SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_NONE);
    }
